...
def preprocess_save_to_queue(preprocess_fn, q, list_of_lists, output_files, segs_from_prev_stage, classes,
    ...
    for i, l in enumerate(list_of_lists):
        try:
            ...
            # print("preprocessing", output_file)  # 05/07
    ...
    if len(errors_in) > 0:
        ...
        # print("This worker has ended successfully, no errors to report")  # 05/07
        print('Success...')  # 05/07
    # restore output
...
def predict_cases(model, list_of_lists, output_filenames, folds, save_npz, num_threads_preprocessing,
    ...
    # print("starting preprocessing generator")  # 05/07
    preprocessing = preprocess_multithreaded(trainer, list_of_lists, cleaned_output_files, num_threads_preprocessing,
                                             segs_from_prev_stage)
    # print("starting prediction...")  # 05/07
    all_output_files = []
...
def predict_cases_fastest(model, list_of_lists, output_filenames, folds, num_threads_preprocessing,
    ...
    if not overwrite_existing:
        # print("number of cases:", len(list_of_lists))  # 05/07
        not_done_idx = [i for i, j in enumerate(cleaned_output_files) if not isfile(j)]
        ...
        # print("number of cases that still need to be predicted:", len(cleaned_output_files))  # 05/07

    # print("emptying cuda cache")  # 05/07
    torch.cuda.empty_cache()

    # print("loading parameters for folds,", folds)  # 05/07
    trainer, params = load_model_and_checkpoint_files(model, folds, mixed_precision=mixed_precision,
                                                      checkpoint_name=checkpoint_name)
    ...
    # print("starting prediction...")  # 05/07
    for preprocessed in preprocessing:
        # print("getting data from preprocessor")  # 05/07
        output_filename, (d, dct) = preprocessed
        # print("got something")  # 05/07
        if isinstance(d, str):
            print("what I got is a string, so I need to load a file")
            data = np.load(d)
            os.remove(d)
        ...
        all_seg_outputs = np.zeros((len(params), *d.shape[1:]), dtype=int)
        # print("predicting", output_filename)  # 05/07
        ...
        for i, p in enumerate(params):
            ...
            res = trainer.predict_preprocessed_data_return_seg_and_softmax(d, do_mirroring=do_tta,
                                                                           ...
                                                                           all_in_gpu=all_in_gpu,
                                                                           mixed_precision=mixed_precision,
                                                                           verbose= False  # ban variable print
                                                                           )
            if len(params) > 1:
                # otherwise we dont need this and we can save ourselves the time it takes to copy that
        ...
        # print("aggregating predictions")  # 05/07
        if len(params) > 1:
        ...
        # print("applying transpose_backward")  # 05/07
        transpose_forward = trainer.plans.get('transpose_forward')
        if transpose_forward is not None:
            transpose_backward = trainer.plans.get('transpose_backward')
            seg = seg.transpose([i for i in transpose_backward])

        # print("initializing segmentation export")  # 05/07
        results.append(pool.starmap_async(save_segmentation_nifti,
                                          ((seg, output_filename, dct, 0, None),)
                                          ))
        print("done")

    # print("inference done. Now waiting for the segmentation export to finish...")  # 05/07
    _ = [i.get() for i in results]
    # now apply postprocessing
    # first load the postprocessing properties if they are present. Else raise a well visible warning
    if not disable_postprocessing:
...
def check_input_folder_and_return_caseIDs(input_folder, expected_num_modalities):
    # print("This model expects %d input modalities for each image" % expected_num_modalities)  # 05/07
