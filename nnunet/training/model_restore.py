...
def restore_model(pkl_file, checkpoint=None, train=False, fp16=None):
    ...
    init = info['init']
    name = info['name']  # 'nnUNetTrainerV2'
    # search_in = join(nnunet.__path__[0], "training", "network_training")  # '/home/yuzq/uCode/2025/brainseg/./nnunet/training/network_training'
    # # tr: <class 'nnunet.training.network_training.nnUNetTrainerV2.nnUNetTrainerV2'>
    # tr = recursive_find_python_class([search_in], name, current_module="nnunet.training.network_training")
    # 2025/04/30 No more searching in 'network_training' folder, just import. To avoid pyinstaller error.
    tr = nnUNetTrainerV2
...
def load_model_and_checkpoint_files(folder, folds=None, mixed_precision=None, checkpoint_name="model_best"):
    ...
    if isinstance(folds, str):
        ...
        # print("folds is None so we will automatically look for output folders (not using \'all\'!)")  # 05/07
        folds = subfolders(folder, prefix="fold")
        # print("found the following folds: ", folds)  # 05/07
    else:
    ...
    # print("using the following model files: ", all_best_model_files) # 05/07
    all_params = [torch.load(i, map_location=torch.device('cpu')) for i in all_best_model_files]  # Here to load weight 05/07
    for ckpt in all_params:  # 反向操作：把之前减去的 0.5 加回来
        ckpt["state_dict"]["conv_blocks_localization.1.1.blocks.0.conv.weight"] += 0.5
        print("Loaded model")
